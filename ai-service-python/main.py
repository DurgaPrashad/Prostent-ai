...
app = FastAPI(title="Prostent Voice AI Assistant - Powered by Murf and Gemini")
...
SYSTEM_PROMPT = """You are an intelligent voice-first AI assistant for Prostent.
You help users analyze data and provide insights through natural voice interactions powered by Murf API.
...
class GeminiAIEngine:
    ...
    def get_context_aware_prompt(self, data_type, columns, name):
        ...
        if data_type == "Automotive/Vehicle":
            return f"""You are an intelligent voice-first AI assistant for Prostent.
...
Always use specific data from these columns to answer questions."""
            ...
            return f"""You are an intelligent voice assistant for Prostent.
...
def root():
    return {
        "message": "Prostent Voice AI Assistant API - Powered by Murf and Gemini 2.0 Flash",
        "status": "active",
        "version": "2.0.0",
        "model": "gemini-2.0-flash-exp"
    }
